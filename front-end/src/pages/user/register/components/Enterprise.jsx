...
class RegisterForPerson extends Register {
  ...
  firstForm = () => {
    ...
    const handleNext = (e) => {
      ...
      form.validateFields(
        ...
        (err, values) => {
          if (!err) {
            this.setState({ currentStep: 1, formData: {
  ...
  secondForm = () => {
    ...
    const handleNext = (e) => {
      e.preventDefault();
      form.validateFields(
        ['foundation_date', 'register_capital', 'corporate_capital', 'address', 'legal_person_name'],
        (err, values) => {
          if (!err) {
            const verifyRes = comVerify({
              name: formData['name'], legalPerson: values['legal_person_name'],
              regLocation: values['address'], regCapital: values['register_capital']
            });
            if (verifyRes) {
  ...
  finalForm = () => {
    ...
    const getCaptcha = async (e) => {
      ...
      const ok = await legalPersonAuthenByPhone(
        formData['legal_person_name'], result.front.code, form.getFieldValue('contact')
      );
      if (ok) {
  ...
  getStepForm = (currentStep) => {
    switch (currentStep) {
      case 0: return this.firstForm();
      case 1: return this.secondForm();
      case 2: return this.finalForm();
    }
  };
